...
const CLAWGOD_FEATURES_META = {
  ...
  "growthbook-env-overrides-graph": [
    "features-config"
  ],
  ...
  "update-redirect": [
    "update-command-redirect"
  ],
  "macos-cmdv-image-paste": [
    "macos-image-paste"
  ],
  "provider-config": [
    "provider-config"
...
const patches = [
  ...
  },
  {
    ...
    replacer: (m, fn, next, method, nextMethod) => fn
      ? `function ${fn}(){return ${gate('growthbook-config-overrides')}?null:void 0}${next}`
      : `${method}(){return ${gate('growthbook-config-overrides')}?null:void 0}${nextMethod}(`,
    selectIndex: 0,
    validate: (match, code) => {
